...
!!!!!!!!!!!****************** Initial Guess block *****************!!!!!!
if (useold == 1) then
    open(unit=57,file='../model_input/initial_bench.txt',status="old",action="read",position="rewind")
        read(unit=57, fmt=*)kk    
        read(unit=57, fmt=*)ll
...
open(unit=10, file="../results/output_bench.txt", status="replace", action="write", position="rewind")
...
do while ( (((kdev > tolk) .or. (cdev > tolc)) .and. (iter < maxiter) ) .or. (iter == 1))
    ...
    open(unit=10, file="../results/output_bench.txt", status="old", action="write", position="append")
    write(unit=10,fmt=*)"         "
...
open(unit=57, file = '../model_input/initial_bench.txt', status="replace", action="write", position="rewind")
...
open(unit=33, file = '../results/simu_data_bench', status="replace", action="write", position="rewind")
...
do ir = 1, nr
    write(unit=33,fmt=*) beqdis(ir)
end do

do iage = 1,nage
    do iab = 1,niab
        do ir = 1,nr
            write(unit=33,fmt=*)muraceedu(iage,iab,ir)
        end do
    end do
end do
...
do iage = 1, nage
    do ir = 1, nr
        write(unit=33,fmt=*)drugpro(iage,ir)
    end do
end do
...
do iab = 1,niab
    do ir = 1,nr
        do ie = 1, neta
            do ie_p = 1, neta
                write(unit=33,fmt=*)pieta(iab,ir,ie,ie_p)
            end do
        end do
    end do
end do
...
do iage = 1,nage
    do is = 1, ns
        do is_p = 1, ns
            do ir = 1, nr
                write(unit=33,fmt=*) pis(iage,is,is_p,ir)
            end do
        end do
    end do
end do
...
do is = 1, ns
    do ir = 1, nr
        write(unit=33,fmt=*) erate1(is,ir)
    end do
end do
...
do iab = 1, niab
    do ir = 1, nr
        do ie_pp = 1, 2
            write(unit=33,fmt=*) ie_pro(iab,ir,ie_pp)
        end do
    end do
end do
...
do iage = 1,nage
    do iab = 1, niab
        do is = 1, ns
            do ie = 1, neta
                do ir = 1, nr
                    write(unit=33,fmt=*)y(iage,iab,is,ie,ir)
                    do ia = 0, na
                        do ii = 1, 5
                            write(unit=33,fmt=*)pie(iage,ia,iab,is,ie,ir,ii)
                            do ip = 0, 1
                                write(unit=33,fmt=*)aplus(iage,ia,iab,is,ie,ir,ii,ip)
                                write(unit=33,fmt=*)cons(iage,ia,iab,is,ie,ir,ii,ip)
        ...
        do iab = 1, niab
            do is = 1, ns
                do ie = 1, neta
                    do ir = 1, nr
                        do ip = 0, 1
                            write(unit=33,fmt=*)cr(iage,ia,iab,is,ie,ir,ip)
                        end do
                    end do
                end do
            end do
...
open(unit=34, file = '../results/incargroup', status="replace", action="write", position="rewind")
do i = 1,10
    write(unit=34,fmt=*)incargroup(i)
end do
write(*,*)'Finish incarceration age groups'


open(unit=35, file = '../results/vic_pro', status="replace", action="write", position="rewind")
do i = 1,5
    write(unit=35,fmt=*)vic(i)
end do

do ii = 1, 5
    write(unit=35,fmt=*)vic(ii)*prorate/scale_factor
end do
write(*,*)'Finish victimization probablity'
