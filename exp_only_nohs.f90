...
! exp: same labor eff (only less than high school is equalized)
eff(:,1,1) = eff(:,1,2)
!!!!!*********************************************************************!!!!!!!
...
!!!!!!!!!!!****************** Initial Guess block *****************!!!!!!
if (useold == 1) then
    open(unit=57,file='../model_input/initial_onlynohs.txt',status="old",action="read",position="rewind")
        read(unit=57, fmt=*)kk    
        read(unit=57, fmt=*)ll
...
open(unit=10, file="../results/output_onlynohs.txt", status="replace", action="write", position="rewind")
...
do while ( (((kdev > tolk) .or. (cdev > tolc)) .and. (iter < maxiter) ) .or. (iter == 1))
    ...
    open(unit=10, file="../results/output_onlynohs.txt", status="old", action="write", position="append")
    write(unit=10,fmt=*)"         "
...
open(unit=57, file = '../model_input/initial_onlynohs.txt', status="replace", action="write", position="rewind")
...
open(unit=33, file = '../results/simu_data_onlynohs', status="replace", action="write", position="rewind")
