...
! exp: same survival probability
psi(:,:,1) = psi(:,:,2)
!!!!!*********************************************************************!!!!!!!

!!!!!!!!!!!********************Population structure block *********************!!!!!!

! exp: same survival probability and edu
open(unit=31, file = '../model_input/pop_morg_sameall.txt', status="old", action="read", position="rewind")
do iage = 1,nage
...
!!!!!!!!!!!****************** Initial Guess block *****************!!!!!!
if (useold == 1) then
    open(unit=57,file='../model_input/initial_sameall.txt',status="old",action="read",position="rewind")
        read(unit=57, fmt=*)kk    
        read(unit=57, fmt=*)ll
...
open(unit=10, file="../results/output_sameall.txt", status="replace", action="write", position="rewind")
...
do while ( (((kdev > tolk) .or. (cdev > tolc)) .and. (iter < maxiter) ) .or. (iter == 1))
    ...
    open(unit=10, file="../results/output_sameall.txt", status="old", action="write", position="append")
    write(unit=10,fmt=*)"         "
...
open(unit=57, file = '../model_input/initial_sameall.txt', status="replace", action="write", position="rewind")
...
open(unit=33, file = '../results/simu_data_sameall', status="replace", action="write", position="rewind")
