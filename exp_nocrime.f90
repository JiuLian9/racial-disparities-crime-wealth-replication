...
! exp: no crime
real*8, parameter :: cbar  = 1d-12 ! consumption for incarcerated person (c_bar)
! real*8, parameter :: gamma = 0.064d0 ! fraction of stolen income
! exp: no crime
real*8, parameter :: gamma = 1d-12! fraction of stolen income
...
!!!!!!!!!!!****************** Initial Guess block *****************!!!!!!
if (useold == 1) then
    open(unit=57,file='../model_input/initial_nocrime.txt',status="old",action="read",position="rewind")
        read(unit=57, fmt=*)kk    
        read(unit=57, fmt=*)ll
...
open(unit=10, file="../results/output_nocrime.txt", status="replace", action="write", position="rewind")
...
do while ( (((kdev > tolk) .or. (cdev > tolc)) .and. (iter < maxiter) ) .or. (iter == 1))
    ...
    open(unit=10, file="../results/output_nocrime.txt", status="old", action="write", position="append")
    write(unit=10,fmt=*)"         "
...
open(unit=57, file = '../model_input/initial_nocrime.txt', status="replace", action="write", position="rewind")
...
open(unit=33, file = '../results/simu_data_nocrime', status="replace", action="write", position="rewind")
