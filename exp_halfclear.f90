...
! exp: half clearance
real*8, parameter :: piep = 0.08585d0 ! probabiliry the criminal is caught (pi_a) 1996-2011
...
!!!!!!!!!!!****************** Initial Guess block *****************!!!!!!
if (useold == 1) then
    open(unit=57,file='../model_input/initial_halfclear.txt',status="old",action="read",position="rewind")
        read(unit=57, fmt=*)kk    
        read(unit=57, fmt=*)ll
...
open(unit=10, file="../results/output_halfclear.txt", status="replace", action="write", position="rewind")
...
do while ( (((kdev > tolk) .or. (cdev > tolc)) .and. (iter < maxiter) ) .or. (iter == 1))
    ...
    open(unit=10, file="../results/output_halfclear.txt", status="old", action="write", position="append")
    write(unit=10,fmt=*)"         "
...
open(unit=57, file = '../model_input/initial_halfclear.txt', status="replace", action="write", position="rewind")
...
open(unit=33, file = '../results/simu_data_halfclear', status="replace", action="write", position="rewind")
